...
with open(os.path.join(base_dir, 'git-fame-input.json'), 'r') as f:
    data = json.load(f)
...
def git_clone_or_pull(repo_url, local_path):
    ...
    if os.path.exists(os.path.join(full_local_path, ".git")):
        ...
        try:
            ...
            branches = subprocess.check_output(["git", "branch", "-r"], cwd=full_local_path,
                                               universal_newlines=True).split("\n")
            for branch in branches:
                if branch and not branch.startswith('origin/HEAD'):
                    branch_name = branch.split('/')[-1].strip()
                    subprocess.check_call(["git", "checkout", branch_name], cwd=full_local_path)
        except subprocess.CalledProcessError as e:
            print(f"Error: {e.output}")
...
with open(os.path.join(base_dir, 'output.json'), 'w') as json_file:
